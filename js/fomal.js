...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 云灿の随笔小站")[0];
}
...
$.ajax({
  ...
  data: {
    key: '7Y5BZ-QJ3C5-Q5IIX-IYDNG-P4UF5-ICFEN',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(111.77991652, 37.14086543, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  try {
    ...
    document.getElementById("welcome-info").innerHTML =
        `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的小伙伴，${timeChange}您现在距离小灿约 <span style="color:var(--theme-color)">${dist}</span> 公里，当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
  ...
  debounce(function () {
    new Vue({
      data: function () {
        this.$notify({
          title: "哎嘿！复制成功🍬",
          message: "若要转载最好保留小灿的原文链接哦，给你一个大大的赞！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
function createtime1() {
  var grt = new Date("01/13/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
|___, ||  :  ||  |  /   \\_ |  _  ||  |  |
|     ||     ||  |  \\     ||  |  ||  |  |
|____/  \\__,_||__|__|\\____||__|__||__|__
...
function createtime2() {
  var ascll2 = [`NCC2-036`, `调用前置摄像头拍照成功，识别为「锐刻五代」`, `Photo captured: `, ` 🤪 `];
  ...
  setTimeout(console.log.bind(console, "%c WELCOME %c 欢迎光临，锐刻五代", "color:white; background-color:#23c682", ""));

  setTimeout(
      console.warn.bind(
          console,
          "%c ⚡ Powered by Yuncan %c 你正在访问云灿の随笔小站",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 云灿の随笔小站") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('云灿の随笔小站的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
l = ["非常抱歉，因为不可控原因，博客将于明天停止运营！", "好消息，日本没了！", "美国垮了，原因竟然是川普！", "微软垮了！", "你的电脑已经过载，建议立即关机！", "你知道吗？小灿很喜欢你哦！", "一分钟有61秒哦", "你喜欢的人跟别人跑了！"]
...
if (m == 1 && dd == 15) {//小灿生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝小灿" + (y - 2005).toString() + "岁生日快乐！✨");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 6 && dd == 30) {//小猫咪生日
...
function createtime() {
  ...
  var start = new Date("01/13/2024 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("01/13/2024 00:00:00");
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        ...
        var kd = `<span style="color:red">玩方舟都卡😖</span>`
      } else if (fps <= 25) {
...
<h2>二、字体设置</h2>
<div class="note warning modern"><p>非商免字体未经授权只能个人使用。本站为完全非商业、非盈利性质的网站，平时用于个人学习交流，如有侵权请联系小灿删除，谢谢！ —— 致版权方</p>
</div>
<p id="swfs">
