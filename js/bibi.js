...
function getNew() {
    ...
    fetch(Url + page).then(res => res.json()).then((res) => {
        ...
        nowNum += items.length
        if (page == 1) {
            document.querySelector('.bb-info').innerHTML = '<svg style="width:1.20em;height:1.20em;top:5px;fill:currentColor;overflow:hidden;position:relative"><use xlink:href="#icon-chat"></svg> 小灿的唠叨(' + total + ')'
        }
        page += 1
    }).then(() => {
        bb();
